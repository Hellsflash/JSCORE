    ...
    const app = Sammy('#container', function () {
        ...
        this.post('#/register', (ctx) => {
            ...
            if (!/^[a-zA-Z]{3,}$/g.test(username)) {
                notify.showError('Username must be at least 3 symbols long and to contain only english alphabet characters!');
            } else if (!/^[a-zA-Z\d]{6,}$/g.test(password)) {
        ...
        this.post('#/create/post', (ctx) => {
            ...
            if (title === '') {
                ...
                notify.showError('Url must be valid!')
        ...
        this.get('#/posts', (ctx) => {
            ...
            posts.myPosts(sessionStorage.getItem('username'))
                .then((posts) => {
                    posts.forEach((p, i) => {
                        p.rank = i + 1;
                        p.date = calcTime(p._kmd.ect);
